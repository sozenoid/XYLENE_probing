...
def estimate_dG(mol_representation, ref_dic, NAME=None):
	...
	print "OPTIMISE THE RESULTING COMPLEX USING CP2K (no charge, already included in the prmtop) ({0:4.4f}s)".format(time.time()-t0)
	...
	print "PERFORM THE NMA FOR THE GUEST AND EXTRACT ENTROPY VALUES ({0:4.4f})".format(time.time()-t0)
	...
	print "OBTAINS POLAR AND APOLAR CONTRIBUTIONS TO THE SOLVATION FOR COMPLEX ({0:4.4f})".format(time.time()-t0)
	...
	print "OBTAINS POLAR AND APOLAR CONTRIBUTIONS TO THE SOLVATION FOR GUEST ({0:4.4f}s)".format(time.time()-t0)
...
# 	tar_cmd = "tar -cvzf {0}-ALL-SAVED.tar {1} --exclude='*tar'".format(rdid, to_tar)
# 	#print tar_cmd 
...
def split_smi_list(big_smile_file, n):
	"""
	Parameters
	 ----------
	big_smile_file : str
	...
	Returns
	-------
	will split the file in n parts where in each file, the smallest smiles are on top
	"""
	with open(big_smile_file) as r:
		smiles = [x.strip().split() for x in r.readlines()]
	sorted_smiles = sorted(smiles, key=lambda y: len(y[0]))
	
	part_files = "{}-part-{}.smi"
	wfiles = []
	for i in range(n):
		wfiles.append(open(part_files.format(big_smile_file[:-4], i), "w"))
			
	for i, mol in enumerate(sorted_smiles):
		k = i%n
		wfiles[k].write("{}\t{}\n".format(*mol))
